...
@php($statusLabels=['new'=>'جديدة','saved'=>'محفوظة','confirmed'=>'مؤكدة','cancelled'=>'ملغاة'])
...
<div class="surface-card admin-panel reveal p-3 p-md-4 mb-4">
    <form method="GET" class="row g-2 align-items-end">
        ...
        <div class="col-md-3 col-xl-3"><label class="form-label">الحالة</label><select class="form-select" name="status"><option value="">كل الحالات</option>@foreach($statusLabels as $key=>$label)<option value="{{ $key }}" @selected(request('status')===$key)>{{ $label }}</option>@endforeach</select></div>
