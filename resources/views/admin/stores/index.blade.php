...
<div class="surface-card admin-panel reveal p-3 p-md-4 mb-4">
    ...
    <form method="POST" action="{{ route('admin.stores.store') }}" class="row g-3">
        ...
        <div class="col-6 col-md-3 col-xl-3"><label class="form-label">نوع الاستيراد</label><select class="form-select" name="adapter"><option value="generic">عام</option><option value="shein">SHEIN</option></select></div>
...
<div class="surface-card admin-panel overflow-hidden reveal"><div class="table-responsive"><table class="table table-modern"><thead><tr><th>الموقع</th><th>النطاقات</th><th>العملة</th><th>الحالة</th></tr></thead><tbody>@forelse($stores as $store)<tr><td class="fw-bold">{{ $store->name }}</td><td class="ltr small">{{ implode(', ',$store->domains) }}</td><td>{{ $currencyLabels[strtoupper((string)$store->currency)] ?? $store->currency }}</td><td><span class="status-badge {{ $store->is_active?'status-success':'status-neutral' }}">{{ $store->is_active?'مفعّل':'موقوف' }}</span></td></tr>@empty<tr><td colspan="4" class="text-center text-secondary py-5">لا توجد مواقع.</td></tr>@endforelse</tbody></table></div></div>
@endsection
