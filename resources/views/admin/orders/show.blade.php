...
<div class="ops-workspace">
    <header class="admin-order-hero mb-4">
        <div class="d-flex flex-column flex-xl-row justify-content-between gap-3 align-items-xl-center">
            ...
            <div class="d-flex flex-wrap gap-2"><a class="btn btn-ghost" href="{{ route('admin.orders.index') }}">← كل الطلبات</a><a class="btn btn-navy" href="{{ route('orders.show',$order) }}" target="_blank">عرض العميل</a></div>
        </div>
    </header>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-6 col-xl-3"><div class="dashboard-stat h-100"><span class="metric-label">العربون</span><strong class="metric-value">{{ number_format((float)$order->deposit_amount,2) }}</strong><small>{{ $order->deposit_type ?: 'غير محدد' }}</small></div></div>
        ...
        <div class="col-xxl-8">
            <section class="admin-panel p-3 p-md-4 mb-4">
                <div class="d-flex flex-column flex-md-row justify-content-between gap-2 align-items-md-center mb-3">
                    <div><div class="page-kicker">المراجعة التفصيلية</div><h2 class="h5 panel-title mb-1">منتجات الطلب</h2><p class="small text-secondary mb-0">السعر الأصلي من SHEIN ثابت. المسؤول يقدر يحدد سعر LYD مُراجع مع سبب.</p></div>
                    <span class="status-badge status-primary">{{ $order->items->count() }} منتج</span>
                </div>
                <div class="admin-item-grid">
                    ...
                    <article class="admin-review-item {{ in_array($item->review_status,['unavailable','price_changed','option_issue','rejected'],true)?'has-issue':'' }}">
                        ...
                        <form method="POST" action="{{ route('admin.orders.items.review',[$order,$item]) }}" class="row g-2 mt-3" data-confirm data-confirm-title="حفظ مراجعة المنتج" data-confirm-text="سيتم تسجيل الحركة وإشعار العميل إذا كان هناك تغيير يحتاج رده.">@csrf @method('PATCH')
                            ...
                            <div class="col-md-7"><label class="form-label small fw-bold">سعر LYD المراجع</label><input type="number" step="0.01" min="0" name="reviewed_unit_price_lyd" class="form-control" value="{{ old('reviewed_unit_price_lyd',$item->reviewed_unit_price_lyd) }}" placeholder="اختياري"></div>
            ...
            <section class="admin-panel p-3 p-md-4 mb-4">
                ...
                <form method="POST" action="{{ route('admin.orders.messages.store',$order) }}" class="d-flex flex-column flex-md-row gap-2">@csrf<input class="form-control" name="message" required maxlength="2000" placeholder="اكتب رسالة واضحة للعميل"><button class="btn btn-primary" type="submit">إرسال</button></form>
            </section>
        </div>
                    ...
                    <div class="accordion ops-accordion" id="orderOpsAccordion">
                        <div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#opsStatus">تغيير الحالة</button></h2><div id="opsStatus" class="accordion-collapse collapse show" data-bs-parent="#orderOpsAccordion"><div class="accordion-body">
                            <form method="POST" action="{{ route('admin.orders.status',$order) }}" class="d-grid gap-2" data-confirm data-confirm-title="تأكيد تغيير الحالة" data-confirm-text="سيتم حفظ الحركة والملاحظة في سجل الطلب.">@csrf @method('PATCH')
                                ...
                                <textarea name="reason" class="form-control" rows="3" maxlength="1500" placeholder="ملاحظة على تغيير الحالة (إلزامية للحالات الحساسة)"></textarea>
                                <select name="visibility" class="form-select"><option value="customer">الملاحظة تظهر للعميل</option><option value="internal">ملاحظة داخلية فقط</option></select>
