    ...
    <div class="row g-4 align-items-start">
        <div class="col-xxl-8">
            <section class="admin-panel p-3 p-md-4 mb-4">
                ...
                <div class="review-complete-bar mt-4 border-success-subtle bg-success-subtle">
                    <div>
                        <strong class="text-success">تم إنهاء المراجعة ✓</strong>
                        <div class="small text-secondary">تمت مراجعة جميع المنتجات في {{ $order->review_completed_at->format('Y-m-d H:i') }}. انتقل الآن إلى اعتماد الطلب وتحديد شروط الدفع.</div>
                    </div>
                    <span class="status-badge status-success">المراجعة مكتملة</span>
                </div>
                @else
            ...
            <div class="admin-sticky-rail">
                <section class="admin-panel p-3 mb-3">
                    ...
                    <div class="accordion ops-accordion" id="orderOpsAccordion">
                        ...
                        <div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#opsApprove">اعتماد وشروط الدفع @if($order->deposit_type)<span class="status-badge status-success ms-2">{{ $depositLabels[$order->deposit_type] ?? $order->deposit_type }}</span>@endif</button></h2><div id="opsApprove" class="accordion-collapse collapse" data-bs-parent="#orderOpsAccordion"><div class="accordion-body">
                            <form method="POST" action="{{ route('admin.orders.approve',$order) }}" class="d-grid gap-2" data-confirm data-confirm-title="اعتماد الطلب" data-confirm-text="تأكد من مراجعة كل المنتجات قبل الاعتماد.">@csrf
                                <select name="deposit_mode" class="form-select" required><option value="auto" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='auto')>عربون تلقائي حسب القواعد</option><option value="none" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='none')>بدون عربون</option><option value="percentage" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='percentage')>نسبة مئوية</option><option value="fixed" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='fixed')>مبلغ ثابت</option></select>
                                <input type="number" step="0.01" min="0" name="deposit_value" class="form-control" value="{{ old('deposit_value', $order->deposit_value) }}" placeholder="قيمة النسبة أو المبلغ">
                                <textarea name="decision_note" class="form-control" rows="2" maxlength="1500" placeholder="ملاحظة القبول للعميل (اختيارية)">{{ old('decision_note') }}</textarea>
                                <textarea name="payment_terms_note" class="form-control" rows="2" maxlength="1500" placeholder="ملاحظة شروط الدفع">{{ old('payment_terms_note', $order->payment_terms_note) }}</textarea>
                                <button class="btn btn-primary" type="submit">اعتماد وتحديد الدفع</button>
                            </form>
                        </div></div></div>

                        <div class="accordion-item"><h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#opsTerms">تعديل العربون @if($order->deposit_type)<span class="status-badge status-primary ms-2">{{ $depositLabels[$order->deposit_type] ?? $order->deposit_type }}</span>@endif</button></h2><div id="opsTerms" class="accordion-collapse collapse" data-bs-parent="#orderOpsAccordion"><div class="accordion-body">
                            <form method="POST" action="{{ route('admin.orders.payment-terms',$order) }}" class="d-grid gap-2" data-confirm data-confirm-title="تعديل شروط الدفع" data-confirm-text="سيتم إشعار العميل بالتعديل.">@csrf @method('PATCH')
                                <select name="deposit_mode" class="form-select" required><option value="auto" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='auto')>تلقائي</option><option value="none" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='none')>بدون عربون</option><option value="percentage" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='percentage')>نسبة</option><option value="fixed" @selected(old('deposit_mode', $order->deposit_type ?? 'auto')==='fixed')>مبلغ ثابت</option></select>
                                <input type="number" step="0.01" min="0" name="deposit_value" class="form-control" value="{{ old('deposit_value', $order->deposit_value) }}" placeholder="القيمة">
                                <textarea name="reason" class="form-control" rows="2" required maxlength="1500" placeholder="سبب التعديل">{{ old('reason') }}</textarea>
                                @if($order->payment_terms_note)<div class="form-text">الشروط المحفوظة: {{ $order->payment_terms_note }}</div>@endif
                                <button class="btn btn-ghost" type="submit">تحديث الشروط</button>
                            </form>
                        </div></div></div>
                    </div>
                </section>
