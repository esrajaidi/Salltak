        ...
        <div class="order-progress" aria-label="مراحل الطلب">
            @foreach($progressSteps as $index=>$step)
                <div class="order-progress-step {{ $index < $currentRank ? 'is-done' : ($index === $currentRank ? 'is-current' : '') }}">
                    <span class="order-progress-dot">{{ $index < $currentRank ? '✓' : $step['icon'] }}</span><small>{{ $step['label'] }}</small>
                </div>
            @endforeach
        </div>
    </section>
    ...
    <div class="row g-4 align-items-start">
                        ...
                        <article class="order-item-card premium-order-item {{ $issue?'has-issue':'' }}">
                            <div class="order-item-main">
                                ...
                                <div class="min-w-0 flex-grow-1">
                                    ...
                                    <div class="saved-product-meta d-flex flex-wrap gap-2 mt-2">@if($item->color)<span>اللون: <strong>{{ $item->color }}</strong></span>@endif @if($item->size)<span>المقاس: <strong>{{ $item->size }}</strong></span>@endif @if($item->variant)<span>SKU: <strong class="ltr">{{ $item->variant }}</strong></span>@endif <span>الكمية: <strong>{{ $item->quantity }}</strong></span></div>
                                    <div class="readonly-price mt-3">
                                        <div><small>السعر بالدولار</small><strong class="ltr">{{ number_format((float)$item->unit_price_original,2) }} {{ $item->currency ?: 'USD' }}</strong></div>
        ...
        <aside class="col-xl-4">
            <div class="order-sticky-rail">
                ...
                <section class="surface-card-elevated p-3 p-md-4 reveal is-visible mb-4">
                    ...
                    <form method="POST" action="{{ route('orders.payments.store',$order) }}" enctype="multipart/form-data" class="d-grid gap-3" data-confirm data-confirm-title="تأكيد إرسال الدفعة" data-confirm-text="تأكد من المبلغ وطريقة الدفع قبل الإرسال.">@csrf
                        ...
                        @foreach($paymentMethods as $method)
                            ...
                            <label class="payment-choice-card"><input class="form-check-input mt-1" type="radio" name="payment_method_id" value="{{ $method->id }}" required @checked(old('payment_method_id')==$method->id)><span class="payment-choice-body"><span class="d-flex justify-content-between gap-2"><strong>{{ $method->name }}</strong>@if((float)$method->fee_value>0)<small>{{ $method->fee_type==='percentage'?$method->fee_value.'%':number_format((float)$method->fee_value,2).' د.ل' }}</small>@endif</span>@if($method->instructions)<small class="d-block text-secondary mt-1">{{ $method->instructions }}</small>@endif @if($details)<span class="payment-choice-details mt-2">@foreach($details as $key=>$value)<small><span>{{ $method->customerDetailLabels()[$key] ?? $key }}</span><strong class="ltr">{{ $value }}</strong></small>@endforeach</span>@endif @if($qrImage)<span class="d-block mt-2"><img src="{{ $qrImage }}" alt="QR {{ $method->name }}" class="payment-qr-preview"></span>@endif @if($externalUrl)<span class="d-block mt-2"><a href="{{ $externalUrl }}" target="_blank" rel="noopener" class="btn btn-sm btn-outline-primary" onclick="event.stopPropagation()">فتح بوابة الدفع الخارجية</a></span>@endif</span></label>
                        @endforeach
                        </div></div>
                ...
                <form method="POST" action="{{ route('orders.cancel',$order) }}" data-confirm data-confirm-title="إلغاء الطلب" data-confirm-text="سيتم تسجيل الإلغاء في سجل الطلب." data-confirm-icon="warning">@csrf @method('PATCH')<button class="btn btn-outline-danger w-100" type="submit">إلغاء الطلب</button></form>
                @endif
            </div>
        </aside>
    </div>
