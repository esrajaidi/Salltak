        ...
        <div class="surface-card-elevated reveal is-visible p-3 p-md-4">
            <div class="alert {{ $result->status === 'success' ? 'alert-success' : 'alert-warning' }} border-0 import-alert mb-3">
                ...
                <div class="flex-grow-1">
                    ...
                    @if($result->status === 'success')<div class="small mt-1">تم العثور على <strong>{{ count($result->items) }}</strong> منتج/منتجات من السلة.</div>@endif
                </div>
            </div>
            ...
            @if(!empty($result->meta['group_id']))
                <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 p-3 rounded-4 border bg-light mb-4">
                    <div>
                        ...
                        <div class="fw-bold mt-1">المجموعة <span class="ltr d-inline-block">#{{ $result->meta['group_id'] }}</span></div>
                        @if(!empty($result->meta['local_country']))<div class="small text-secondary mt-1">بلد السلة: {{ $result->meta['local_country'] }}</div>@endif
                    </div>
            ...
            <form method="POST" action="{{ route('carts.store') }}" id="cartForm">
                ...
                <input type="hidden" name="source_currency" value="{{ $currency }}">
                <input type="hidden" name="import_status" value="{{ $result->status }}">
                <input type="hidden" name="import_message" value="{{ $result->message }}">
                ...
                <div class="row g-2 g-md-3 mb-4">
                    ...
                    <div class="col-6 col-lg-3"><div class="summary-tile"><div class="summary-label">العملة</div><div class="summary-value ltr text-end">{{ $currency }}</div></div></div>
                    <div class="col-6 col-lg-3"><div class="summary-tile"><div class="summary-label">سعر الصرف</div><div class="summary-value"><span class="ltr">1 {{ $currency }}</span> = {{ number_format((float)$rate, 4) }} د.ل</div></div></div>
                    <div class="col-6 col-lg-3"><div class="summary-tile"><div class="summary-label">عدد المنتجات</div><div class="summary-value" id="itemsCount">{{ count($result->items) ?: 1 }}</div></div></div>
                </div>
                        ...
                        <article class="product-editor" data-row>
                            <div class="row g-3 align-items-start">
                                ...
                                <div class="col min-w-0">
                                    ...
                                    <label class="visually-hidden" for="item-name-{{ $i }}">اسم المنتج</label>
                                    <input id="item-name-{{ $i }}" class="form-control product-title-input" name="items[{{ $i }}][name]" value="{{ $item['name'] ?? '' }}" placeholder="اسم المنتج" required>

                                    <input type="hidden" name="items[{{ $i }}][external_id]" value="{{ $item['external_id'] ?? '' }}">
                                    <input type="hidden" name="items[{{ $i }}][product_url]" value="{{ $item['product_url'] ?? '' }}">
                                    <input type="hidden" name="items[{{ $i }}][image_url]" value="{{ $item['image_url'] ?? '' }}">

                                    <div class="row g-2 mt-1 product-meta-inputs">
                                        <div class="col-6 col-md-4"><input class="form-control" name="items[{{ $i }}][color]" value="{{ $item['color'] ?? '' }}" placeholder="اللون"></div>
                                        <div class="col-6 col-md-4"><input class="form-control" name="items[{{ $i }}][size]" value="{{ $item['size'] ?? '' }}" placeholder="المقاس"></div>
                                        <div class="col-12 col-md-4"><input class="form-control" name="items[{{ $i }}][variant]" value="{{ $item['variant'] ?? '' }}" placeholder="SKU / الخيار"></div>
                                    </div>

                                    @if(!empty($item['external_id']))<div class="small text-secondary mt-2">ID: <code class="ltr">{{ $item['external_id'] }}</code></div>@endif
                                </div>
                                    ...
                                    <div class="product-price-panel">
                                        <div class="price-pair mb-2">
                                            <div class="price-box">
                                                ...
                                                <div class="d-flex align-items-center gap-1 ltr">
                                                    <input class="form-control form-control-sm price border-0 p-0 shadow-none fw-bold" type="number" step="0.01" min="0" name="items[{{ $i }}][unit_price_original]" value="{{ $item['unit_price_original'] ?? 0 }}" required>
                                        ...
                                        <div class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-end justify-content-between gap-2">
                                            ...
                                            <div class="text-sm-end">
                                                ...
                                                <div class="fw-bold ltr line-total-original">0.00 {{ $currency }}</div>
                ...
                <div class="row g-3 mt-3 align-items-stretch">
                    <div class="col-lg-5 ms-lg-auto">
                        <div class="cart-total-card h-100">
                            <div class="d-flex align-items-center justify-content-between gap-3 mb-2"><span class="summary-label">إجمالي السلة بالعملة الأصلية</span><strong class="ltr" id="subtotal">0.00 {{ $currency }}</strong></div>
